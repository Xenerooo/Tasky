...
export default function GroupCard({ group, onPress, onEdit, onDelete }: GroupCardProps) {
  ...
  const statusColor = isDone ? '#34C759' : isOngoing ? '#FF9500' : isPending ? '#FF3B30' : '#8E8E93';
  ...
  const cardContent = (
    <TouchableOpacity style={styles.card} onPress={() => { closeSlide(); onPress(group); }} activeOpacity={0.7}>
      <View style={styles.header}>
        <Text style={styles.title} numberOfLines={2}>{group.title}</Text>
      </View>
      {noDeadlines ? (
        <View style={styles.statusRow}>
          <Text style={[styles.statusText, { color: statusColor }]}>{group.calculated_status}</Text>
        </View>
      ) : (
        <>
          <View style={styles.progressBarBg}>
            <View style={[styles.progressBarFill, { width: `${group.progress_percentage}%` }]} />
          </View>
          <View style={styles.statusRow}>
